...
// Adapted from scottstts/Threejs-Awesome-Graphics-Agent-Skills (MIT).
import {
  FragmentIFFT,
  createTarget,
  vertexShader,
...
export class SpectralCascade {
  ...
  setIntensity(intensity) {
    this.heightEvolution.uniforms.amplitude.value = intensity;
    this.horizontalEvolution.uniforms.amplitude.value = intensity;
  }
  ...
  dispose() {
    ...
    this.displacementTargets.forEach((target) => target.dispose());
...
export class SpectralOceanSystem {
  ...
  setIntensity(intensity) {
    for (const cascade of this.cascades) cascade.setIntensity(intensity);
  }
